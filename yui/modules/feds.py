...
import Yui.modules.sql.feds_sql as sql
from Yui import (
    EVENT_LOGS,
    LOGGER,
    SUPPORT_CHAT,
...
from Yui.modules.helper_funcs.string_handling import markdown_parser
...
def fed_ban(update: Update, context: CallbackContext):
    ...
    if fban:
        fed_name = info["fname"]
        # https://t.me/YuiSupport/41606 // https://t.me/YuiSupport/41619
    ...
    if args:
        if args[0] == "json":
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "Yui_fbanned_users.json"
                update.effective_message.reply_document(
                    document=output,
                    filename="Yui_fbanned_users.json",
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "Yui_fbanned_users.csv"
                update.effective_message.reply_document(
                    document=output,
                    filename="Yui_fbanned_users.csv",
