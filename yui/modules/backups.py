...
from Yui import dispatcher, LOGGER, OWNER_ID, JOIN_LOGGER, SUPPORT_CHAT
...
# from Yui.modules.rules import get_rules
import Yui.modules.sql.rules_sql as rulessql

# from Yui.modules.sql import warns_sql as warnssql
import Yui.modules.sql.blacklist_sql as blacklistsql
from Yui.modules.sql import disable_sql as disabledsql

# from Yui.modules.sql import cust_filters_sql as filtersql
# import Yui.modules.sql.welcome_sql as welcsql
...
def export_data(update: Update, context: CallbackContext):
    ...
    with open("Yui{}.backup".format(chat_id), "w") as f:
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("Yui{}.backup".format(chat_id), "rb"),
        caption="*Successfully Exported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `Yui-Backup` was specially made for notes.".format(
            chat.title, chat_id, tgl
        ),
        timeout=360,
    ...
    os.remove("Yui{}.backup".format(chat_id))  # Cleaning file
